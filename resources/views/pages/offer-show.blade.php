...
<div class="page-hero" aria-label="{{ $offer->title }}">
    <div class="container" style="position:relative;z-index:1">
        ...
        @if($offer->service)
        <p class="page-hero__sub">{{ $offer->service->localized_name ?? $offer->service->name }}</p>
        @endif
    </div>
</div>
    ...
    <div class="container">
        <div style="display:grid;grid-template-columns:1fr 320px;gap:var(--sp-12);align-items:start">
            ...
            <div>
                ...
                @if($offer->terms)
                <div style="background:rgba(11,21,32,.03);border-radius:10px;padding:var(--sp-6);border-inline-start:3px solid var(--gold-dark)">
                    <h3 style="font-size:.95rem;font-weight:700;color:var(--navy);margin:0 0 var(--sp-3)">{{ __('messages.offer_detail.terms_heading') }}</h3>
            ...
            <div style="position:sticky;top:calc(var(--nav-h, 72px) + var(--sp-6))">
                <div style="border:1px solid var(--border);border-radius:12px;overflow:hidden;background:#fff;box-shadow:0 4px 24px rgba(11,21,32,.07)">
                    ...
                    <div style="background:var(--navy);padding:var(--sp-5);text-align:center">
                        ...
                        <div style="font-size:.8rem;font-weight:600;color:rgba(255,255,255,.7);text-transform:uppercase;letter-spacing:.5px">{{ __('messages.offer_detail.discount_label') }}</div>
                    </div>
                    @endif
                    ...
                    <div style="padding:var(--sp-6)">
                        ...
                        <div style="margin-bottom:var(--sp-5);padding-bottom:var(--sp-5);border-bottom:1px solid var(--border)">
                            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:var(--sp-2)">
                                <span style="font-size:.8rem;color:var(--text-light)">{{ __('messages.offer_detail.original_price') }}</span>
                            ...
                            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:var(--sp-2)">
                                <span style="font-size:.8rem;color:var(--text-light)">{{ __('messages.offer_detail.offer_price') }}</span>
                            ...
                            <div style="display:flex;justify-content:space-between;align-items:center">
                                <span style="font-size:.8rem;color:var(--text-light)">{{ __('messages.offer_detail.you_save') }}</span>
                        ...
                        <div style="display:flex;align-items:center;gap:var(--sp-2);margin-bottom:var(--sp-4);font-size:.85rem;color:var(--text-mid)">
                            ...
                            <span>{{ __('messages.offer_detail.expires') }} {{ $offer->expires_at->format('d M Y') }}</span>
                        </div>
                        @endif
                        ...
                        <a href="{{ route('booking.index') }}{{ $offer->service_id ? '?service=' . $offer->service_id : '' }}"
                           class="btn btn--primary" style="width:100%;text-align:center;display:block;margin-bottom:var(--sp-3)">
                            {{ __('messages.offer_detail.book_btn') }}
                        </a>
                        <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', config('clinic.whatsapp','201000000000')) }}"
                           ...
                           class="btn btn--outline" style="width:100%;text-align:center;display:block">
                            {{ __('messages.offer_detail.ask_wa_btn') }}
                        </a>
