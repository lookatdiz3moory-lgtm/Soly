...
<div class="page-hero" aria-label="{{ __('messages.contact_page.page_heading') }}">
    <div class="container" style="position:relative;z-index:1">
        <div class="page-hero__tag section-tag" style="margin-bottom:var(--sp-4)">{{ __('messages.contact_page.page_tag') }}</div>
        <h1 class="page-hero__title">{{ __('messages.contact_page.page_heading') }}</h1>
        <p class="page-hero__sub">{{ __('messages.contact_page.page_sub') }}</p>
    </div>
</div>
...
<section style="padding:var(--sp-16) 0">
    <div class="container">
        <div style="display:grid;grid-template-columns:1fr 380px;gap:var(--sp-12);align-items:start">
            ...
            {{-- Contact form --}}
            <div>
                <h2 style="font-size:1.4rem;font-weight:700;color:var(--navy);margin-bottom:var(--sp-8)">{{ __('messages.contact_page.form_heading') }}</h2>
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:var(--sp-5);margin-bottom:var(--sp-5)">
                    <div>
                        <label for="name" style="display:block;font-size:.85rem;font-weight:600;color:var(--navy);margin-bottom:var(--sp-2)">
                            {{ __('messages.contact_page.label_name') }} <span style="color:var(--gold-dark)">*</span>
                        </label>
                    ...
                    <div>
                        <label for="phone" style="display:block;font-size:.85rem;font-weight:600;color:var(--navy);margin-bottom:var(--sp-2)">
                            {{ __('messages.contact_page.label_phone') }}
                        </label>
                        <input type="tel" id="phone" name="phone" maxlength="25"
                               value="{{ old('phone') }}"
                               placeholder="{{ __('messages.contact_page.placeholder_phone') }}"
                ...
                <div style="margin-bottom:var(--sp-5)">
                    <label for="email" style="display:block;font-size:.85rem;font-weight:600;color:var(--navy);margin-bottom:var(--sp-2)">
                        {{ __('messages.contact_page.label_email') }}
                    </label>
                    <input type="email" id="email" name="email" maxlength="150"
                           value="{{ old('email') }}"
                           placeholder="{{ __('messages.contact_page.placeholder_email') }}"
                ...
                <div style="margin-bottom:var(--sp-5)">
                    <label for="subject" style="display:block;font-size:.85rem;font-weight:600;color:var(--navy);margin-bottom:var(--sp-2)">
                        {{ __('messages.contact_page.label_subject') }}
                    </label>
                    <input type="text" id="subject" name="subject" maxlength="200"
                           value="{{ old('subject') }}"
                           placeholder="{{ __('messages.contact_page.placeholder_subject') }}"
                           style="width:100%;padding:var(--sp-3) var(--sp-4);border:1px solid var(--border);border-radius:8px;font-size:.9rem;font-family:inherit;outline:none;transition:border-color .2s;box-sizing:border-box">
                </div>
                ...
                <div style="margin-bottom:var(--sp-7)">
                    <label for="message" style="display:block;font-size:.85rem;font-weight:600;color:var(--navy);margin-bottom:var(--sp-2)">
                        {{ __('messages.contact_page.label_message') }} <span style="color:var(--gold-dark)">*</span>
                    </label>
                    <textarea id="message" name="message" required rows="5" maxlength="2000"
                              placeholder="{{ __('messages.contact_page.placeholder_message') }}"
                ...
                <button type="submit" class="btn btn--primary">{{ __('messages.contact_page.send_btn') }}</button>
            ...
            <div style="position:sticky;top:calc(var(--nav-h, 72px) + var(--sp-6))">
                ...
                <div style="border:1px solid var(--border);border-radius:12px;overflow:hidden;background:#fff;box-shadow:0 4px 24px rgba(11,21,32,.07);margin-bottom:var(--sp-5)">
                    <div style="background:var(--navy);padding:var(--sp-5) var(--sp-6)">
                        ...
                        <div style="font-size:.8rem;color:rgba(255,255,255,.6);margin-top:4px">{{ __('messages.contact_page.contact_location') }}</div>
                    </div>
                    ...
                    <div style="padding:var(--sp-5) var(--sp-6);display:grid;gap:var(--sp-4)">
                        ...
                        <a href="tel:{{ config('clinic.phone', '+201005826642') }}"
                           style="display:flex;align-items:center;gap:var(--sp-3);text-decoration:none;color:inherit">
                            ...
                            <div>
                                <div style="font-size:.7rem;font-weight:600;text-transform:uppercase;letter-spacing:.5px;color:var(--text-light)">{{ __('messages.contact_page.contact_phone') }}</div>
                                <div style="font-size:.9rem;font-weight:600;color:var(--navy)" dir="ltr">{{ config('clinic.phone', '+20 100 582 6642') }}</div>
                            </div>
                        </a>
                        ...
                        <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', config('clinic.whatsapp','201000000000')) }}"
                           ...
                           style="display:flex;align-items:center;gap:var(--sp-3);text-decoration:none;color:inherit">
                            ...
                            <div>
                                <div style="font-size:.7rem;font-weight:600;text-transform:uppercase;letter-spacing:.5px;color:var(--text-light)">{{ __('messages.contact_page.contact_whatsapp') }}</div>
                                <div style="font-size:.9rem;font-weight:600;color:var(--navy)">{{ __('messages.contact_page.contact_whatsapp_link') }}</div>
                            </div>
                        </a>
                        ...
                        <a href="mailto:{{ config('clinic.email') }}"
                           style="display:flex;align-items:center;gap:var(--sp-3);text-decoration:none;color:inherit">
                            ...
                            <div>
                                <div style="font-size:.7rem;font-weight:600;text-transform:uppercase;letter-spacing:.5px;color:var(--text-light)">{{ __('messages.contact_page.contact_email') }}</div>
                        ...
                        <div style="display:flex;align-items:flex-start;gap:var(--sp-3)">
                            <span style="width:36px;height:36px;border-radius:50%;background:rgba(201,168,76,.12);display:flex;align-items:center;justify-content:center;font-size:16px;flex-shrink:0" aria-hidden="true">📍</span>
                            <div>
                                <div style="font-size:.7rem;font-weight:600;text-transform:uppercase;letter-spacing:.5px;color:var(--text-light)">{{ __('messages.contact_page.contact_address') }}</div>
                ...
                <a href="{{ route('booking.index') }}"
                   class="btn btn--primary" style="width:100%;text-align:center;display:block">
                    {{ __('messages.contact_page.book_btn') }}
                </a>
